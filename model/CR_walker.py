...
class ProRec(nn.Module):
    ...
    def __init__(self, device_str='cuda:1', rnn_type="RNN_TANH", use_bert=True, utter_embed_size=64, dropout=0.5,
        ...
        self.utter_embedder = Utterance_Embedder(rnn_type, use_bert, utter_embed_size, dropout, num_turns,
                                                 word_net=word_net)
        self.graph_embedder = Graph_Embedder(num_nodes=self.num_nodes, embed_size=graph_embed_size,
                                             device_str=device_str, word_net=word_net)
        self.intent_selector = IntentSelector(utter_embed_size, atten_hidden)
        self.graph_walker = Graph_Walker(attention_hidden_dim=atten_hidden, graph_embed_size=graph_embed_size,
                                         utterance_embed_size=utter_embed_size, device_str=device_str,
                                         nagetive_sample_ratio=negative_sample_ratio, word_net=word_net)
        if self.dataset == "gorecdial":
            self.explicit_recommender = Explicit_Recommender(utterance_embed_size=utter_embed_size,
                                                             graph_embed_size=graph_embed_size)  # utterance_embed_size=utter_embed_size,graph_embed_size=graph_embed_size)
    ...
    def prepare_reg(self, mention_history, dialog_history, intent=None, rec_cand=None):
        ...
        if self.word_net:
            tokenized_dialog, all_length, maxlen, init_hidden, word_index, word_batch_index, raw_history = self.utter_embedder.prepare_data(
                dialog_history, self.device, raw_history=True)
    ...
    def forward(self, tokenized_dialog, all_length, maxlen, init_hidden, edge_type, edge_index, mention_index,
                mention_batch_index, sel_indices, sel_batch_indices, sel_group_indices, grp_batch_indices, last_indices,
                intent_indices, intent_label, label_1, label_2, score_masks, alignment_index, alignment_batch_index,
                alignment_label, word_index=None, word_batch_index=None, alignment_index_word=None,
                alignment_batch_index_word=None, alignment_label_word=None):
        utter_embed = self.utter_embedder.forward(tokenized_dialog, all_length, maxlen, init_hidden)
        ...
        paths = self.graph_walker.forward(graph_embed, utter_embed, mention_index, mention_batch_index, sel_indices,
                                          ...
                                          intent_indices, score_masks, word_embed=word_embed,
    ...
    def forward_gorecdial(self, tokenized_dialog, all_length, maxlen, init_hidden, edge_type, edge_index, mention_index,
                          mention_batch_index, sel_indices, sel_batch_indices, sel_group_indices, grp_batch_indices,
                          last_indices, intent_indices, bow_embed, rec_index, rec_batch_index, rec_golden, intent_label,
                          label_1, label_2, score_masks, alignment_index, alignment_batch_index, alignment_label,
                          word_index=None, word_batch_index=None, alignment_index_word=None,
                          alignment_batch_index_word=None, alignment_label_word=None):
        ...
        paths, user_portrait = self.graph_walker.forward(graph_embed, utter_embed, mention_index, mention_batch_index,
                                                         sel_indices, sel_batch_indices, sel_group_indices,
                                                         grp_batch_indices, last_indices, intent_indices, score_masks,
                                                         ret_portrait=True, word_embed=word_embed,
                                                         word_batch_index=word_batch_index, word_index=word_index)

        rec = self.explicit_recommender.forward(utter_embed, graph_embed, user_portrait, bow_embed, rec_index,
                                                rec_batch_index)
    ...
    def inference_gorecdial(self, intent, tokenized_dialog, all_length, maxlen, init_hidden, edge_type, edge_index,
                            mention_index, mention_batch_index, sel_index, sel_batch_index, sel_group_index,
                            grp_batch_index, last_index, score_mask, word_index, word_batch_index, bow_embed=None,
                            sel_index_ex=None, sel_batch_index_ex=None, last_weights=None, layer=0):
        utter_embed = self.utter_embedder.forward(tokenized_dialog, all_length, maxlen, init_hidden)
        # print("utterance_embedding:",utter_embed.size())
        graph_embed, word_embed = self.graph_embedder.forward(edge_type, edge_index)

        user_portrait = self.graph_walker.get_user_portrait(mention_index, mention_batch_index, graph_embed, word_index,
                                                            word_batch_index, word_embed)

        step, weight, partial_score = self.graph_walker.forward_single_layer(layer, utter_embed, user_portrait,
                                                                             graph_embed, sel_index, sel_batch_index,
                                                                             sel_group_index, grp_batch_index,
                                                                             last_index, intent, score_mask,
                                                                             last_weights, True)
        if bow_embed != None:
            rec = self.explicit_recommender.forward(utter_embed, graph_embed, user_portrait, bow_embed, sel_index_ex,
                                                    sel_batch_index_ex)
            return step, weight, partial_score, rec
        else:
            return step, weight, partial_score

    def inference_redial(self, intent, tokenized_dialog, all_length, maxlen, init_hidden, edge_type, edge_index,
                         mention_index, mention_batch_index, sel_index, sel_batch_index, sel_group_index,
                         grp_batch_index, last_index, score_mask, word_index, word_batch_index, last_weights=None,
                         layer=0):
        utter_embed = self.utter_embedder.forward(tokenized_dialog, all_length, maxlen, init_hidden)
        # print("utterance_embedding:",utter_embed.size())
        graph_embed, word_embed = self.graph_embedder.forward(edge_type, edge_index)
        ...
        step, weight, partial_score = self.graph_walker.forward_single_layer(layer, utter_embed, user_portrait,
                                                                             ...
                                                                             sel_group_index, grp_batch_index,
                                                                             last_index, intent, score_mask,
                                                                             last_weights, ret_partial_score=True)
        return step, weight, partial_score

    def prepare_data_redial(self, dialog_history, mention_history, intent, node_candidate1, node_candidate2, edge_type,
                            edge_index, label1, label2, gold_pos, attribute_dict, sample=False):
        tokenized_dialog, all_length, maxlen, init_hidden, word_index, word_batch_index = self.utter_embedder.prepare_data(
            dialog_history, self.device)
        mention_index, mention_batch_index, sel_indices, sel_batch_indices, sel_group_indices, grp_batch_indices, last_indices, intent_indices, label_1, label_2, score_masks = self.graph_walker.prepare_data(
            mention_history, intent, node_candidate1, node_candidate2, label1, label2, attribute_dict, self.device,
            gold_pos, sample=sample, dataset="redial")
        edge_index = edge_index.to(device=self.device)
        edge_type = edge_type.to(device=self.device)
        all_intent = ["chat", "question", "recommend"]
    ...
    def prepare_data_interactive(self, dialog_history, mention_history, edge_type, edge_index):
        ...
        tokenized_dialog, all_length, maxlen, init_hidden, _, _ = self.utter_embedder.prepare_data(dialog_history,
                                                                                                   self.device)
    ...
    def prepare_data_gorecdial(self, dialog_history, mention_history, intent, node_candidate1, node_candidate2,
                               edge_type, edge_index, label1, label2, attribute_dict, rec_cand, all_bows):
        tokenized_dialog, all_length, maxlen, init_hidden, word_index, word_batch_index = self.utter_embedder.prepare_data(
            dialog_history, self.device)
        mention_index, mention_batch_index, sel_indices, sel_batch_indices, sel_group_indices, grp_batch_indices, last_indices, intent_indices, label_1, label_2, score_masks = self.graph_walker.prepare_data(
            mention_history, intent, node_candidate1, node_candidate2, label1, label2, attribute_dict, self.device,
            sample=False, dataset="gorecdial")
        edge_index = edge_index.to(device=self.device)
